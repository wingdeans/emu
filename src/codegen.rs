...
impl Codegen {
    ...
    fn emit_varnode(&mut self, text: &str, idx: usize) {
        let name = format_ident!("Sym{}", idx);
    ...
    fn emit_varlist(&mut self, varlist: Varlist, idx: usize) {
        let name = format_ident!("Sym{}", idx);
...
pub(crate) fn emit(sleigh: SymbolTable) -> TokenStream {
    ...
    let mut cg = Codegen(tokens);

    for (i, sym) in sleigh.syms.into_iter().enumerate() {
        match sym {
            Sym::Subtable(subtable) => cg.emit_subtable(subtable, i),
            Sym::Op(operand) => cg.emit_operand(operand, i),
            Sym::Varnode => cg.emit_varnode(&sleigh.sym_names[i], i),
            Sym::Varlist(varlist) => cg.emit_varlist(varlist, i),
            _ => (),
        }
    }
